        ...
        if (!empty($users)) {
            ...
            foreach ($users as $uid => $user) {
                if (!empty($user['assign'])) {
                    ...
                    $insrecord['assigntime'] = pm_gmt_from_usertime(mktime(0, 0, 0, $startmonth, $startday, $startyear));

                    $endyear  = $user['endyear'];
                    $endmonth = $user['endmonth'];
                    $endday   = $user['endday'];
                    $insrecord['completetime'] = pm_gmt_from_usertime(mktime(0, 0, 0, $endmonth, $endday, $endyear));
