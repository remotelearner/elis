 ...
 */
class coursecorequisiteTest extends PHPUnit_Framework_TestCase {
    ...
    protected function load_csv_data() {
        ...
        $dataset->addTable(coursecorequisite::TABLE, elis::component_file('program', 'phpunit/coursecorequisite.csv')); // TBD: more generic 'phpunit/' . get_class($this) ???
     ...
     */
    public function testCourseCorequisiteHasCorrectDBFields() {
        $testobj = new coursecorequisite(false, null, array(), false, array(), self::$origdb);
     ...
     */
    public function testCourseCorequisiteHasCorrectAssociations() {
        $testobj = new coursecorequisite(false, null, array(), false, array(), self::$origdb);
     ...
     */
    public function testCourseCorequisiteCanCreateRecord() {
        $this->markTestIncomplete('In development.');
        // create a record
        $src = new coursecorequisite(false, null, array(), false, array(), self::$overlaydb);
        // TBD: setup data
        $src->save();

        // read it back
        $retr = new coursecorequisite($src->id, null, array(), false, array(), self::$overlaydb);
     ...
     */
    public function testCourseCorequisiteCanUpdateRecord() {
        $this->markTestIncomplete('In development.');
        $this->load_csv_data();

        // read a record
        $src = new coursecorequisite(3, null, array(), false, array(), self::$overlaydb);
        // TBD: setup data
        $src->save();

        // read it back
        $retr = new coursecorequisite(3, null, array(), false, array(), self::$overlaydb);
        foreach ($src as $key => $value) {
            if (strpos($key, elis_data_object::FIELD_PREFIX) !== false) {
                $key = substr($key, strlen(elis_data_object::FIELD_PREFIX));
                $this->assertEquals($src->{$key}, $retr->{$key});
