...
class coursecorequisiteTest extends PHPUnit_Framework_TestCase {
    ...
    public function testCourseCorequisiteHasCorrectDBFields() {
        $testobj = new coursecorequisite(false, null, array(), false, array(), $this->origdb);
    ...
    public function testCourseCorequisiteCanCreateRecord() {
        ...
        // create a record
        $src = new coursecorequisite(false, null, array(), false, array(), $this->overlaydb);
        // TBD: setup data
        $src->save();

        // read it back
        $retr = new coursecorequisite($src->id, null, array(), false, array(), $this->overlaydb);
    ...
    public function testCourseCorequisiteCanUpdateRecord() {
        ...
        // read a record
        $src = new coursecorequisite(3, null, array(), false, array(), $this->overlaydb);
        // TBD: setup data
        $src->save();

        // read it back
        $retr = new coursecorequisite(3, null, array(), false, array(), $this->overlaydb);
        foreach ($src as $key => $value) {
            if (strpos($key, elis_data_object::FIELD_PREFIX) !== false) {
                $key = substr($key, strlen(elis_data_object::FIELD_PREFIX));
                $this->assertEquals($src->{$key}, $retr->{$key});
