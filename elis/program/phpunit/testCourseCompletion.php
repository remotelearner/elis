 ...
 */
class coursecompletionTest extends PHPUnit_Framework_TestCase {
    ...
    protected function load_csv_data() {
        ...
        $dataset->addTable(coursecompletion::TABLE, elis::component_file('program', 'phpunit/coursecompletion.csv')); // TBD: more generic 'phpunit/' . get_class($this) ???
     ...
     */
    public function testCourseCompletionHasCorrectDBFields() {
        $testobj = new coursecompletion(false, null, array(), false, array(), $this->origdb);
     ...
     */
    public function testCourseCompletionHasCorrectAssociations() {
        $testobj = new coursecompletion(false, null, array(), false, array(), $this->origdb);
     ...
     */
    public function testCourseCompletionCanCreateRecord() {
        $this->markTestIncomplete('In development.');
        // create a record
        $src = new coursecompletion(false, null, array(), false, array(), $this->overlaydb);
        // TBD: setup data
        $src->save();

        // read it back
        $retr = new coursecompletion($src->id, null, array(), false, array(), $this->overlaydb);
     ...
     */
    public function testCourseCompletionCanUpdateRecord() {
        $this->markTestIncomplete('In development.');
        $this->load_csv_data();

        // read a record
        $src = new coursecompletion(3, null, array(), false, array(), $this->overlaydb);
        // TBD: setup data
        $src->save();

        // read it back
        $retr = new coursecompletion(3, null, array(), false, array(), $this->overlaydb);
        foreach ($src as $key => $value) {
            if (strpos($key, elis_data_object::FIELD_PREFIX) !== false) {
                $key = substr($key, strlen(elis_data_object::FIELD_PREFIX));
                $this->assertEquals($src->{$key}, $retr->{$key});
