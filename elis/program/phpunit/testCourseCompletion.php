...
class coursecompletionTest extends PHPUnit_Framework_TestCase {
    ...
    public function testCourseCompletionHasCorrectDBFields() {
        $testobj = new coursecompletion(false, null, array(), false, array(), self::$origdb);
    ...
    public function testCourseCompletionCanCreateRecord() {
        ...
        // create a record
        $src = new coursecompletion(false, null, array(), false, array(), self::$overlaydb);
        // TBD: setup data
        $src->save();

        // read it back
        $retr = new coursecompletion($src->id, null, array(), false, array(), self::$overlaydb);
    ...
    public function testCourseCompletionCanUpdateRecord() {
        ...
        // read a record
        $src = new coursecompletion(3, null, array(), false, array(), self::$overlaydb);
        // TBD: setup data
        $src->save();

        // read it back
        $retr = new coursecompletion(3, null, array(), false, array(), self::$overlaydb);
        foreach ($src as $key => $value) {
            if (strpos($key, elis_data_object::FIELD_PREFIX) !== false) {
                $key = substr($key, strlen(elis_data_object::FIELD_PREFIX));
                $this->assertEquals($src->{$key}, $retr->{$key});
