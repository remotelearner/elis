...
/////////////////////////////////////////////////////////////////////
     ...
     * Print the paging footer for the table.
     *
     * @param  $exportformat the export format of report or null for any.
     * @return string Column section of query based on the columns already
     *                       registered via add_column
     */
    function get_select_columns($exportformat = null) {

        $list = array();

        foreach($this->headers as $key => $value) {
            if ($exportformat && !in_array($exportformat,
                                           $this->columnexportformats[$key])) {
                continue;
            }
            $list[] = $key;
        }
     ...
     */
    function get_complete_sql_query($use_limit = true, $exportformat = null) {
        $columns = $this->get_select_columns($exportformat);
     ...
     *
     */
    function get_data($exportformat = null) {
        global $CFG, $DB;
    ...
    function render_report($id=0) {
        $this->get_data(php_report::$EXPORT_FORMAT_HTML);
    ...
    if (!empty($table->data)) {
        ...
        foreach ($table->data as $key => $row) {
            ...
            if ($row == 'hr' and $countcols) {
                ...
                if (!isset($columns_header_output)) {
                    continue; // TBD
                }
