...
if ($data = $form->get_data()) {
    if (isset($data->submitbutton)) {
        ...
        echo $OUTPUT->heading(get_string('mappingssaved', 'rlipimport_version1elis'));
...
if ($resetmessage) {
    ...
    echo $OUTPUT->heading(get_string('mappingsreset', 'rlipimport_version1elis'));
}

$cancelmessage = optional_param('cancelmessage', 0, PARAM_INT);
if ($cancelmessage) {
    //notify the user that the settings were reset to DB values
    echo $OUTPUT->heading(get_string('mappingscancelled', 'rlipimport_version1elis'));
}
