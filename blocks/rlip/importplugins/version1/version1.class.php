...
class rlip_importplugin_version1 extends rlip_importplugin_base {
    ...
    function validate_core_user_data($action, $record, $filename) {
        ...
        if (!$this->validate_fixed_list($record, 'auth', array_keys($auths))) {
            $this->fslogger->log_failure("auth value of \"{$record->auth}\" is not a valid auth plugin.", 0, $filename, $this->linenumber, $record, "user");
            return false;
        }
        ...
        if (isset($record->password)) {
            ...
            if (!check_password_policy($record->password, $errmsg)) {
                $this->fslogger->log_failure("password value of \"{$record->password}\" does not conform to your site's password policy.", 0, $filename, $this->linenumber, $record, "user");
                return false;
            }
        }
        ...
        if (!$this->validate_fixed_list($record, 'maildigest', array(0, 1, 2))) {
            $this->fslogger->log_failure("maildigest value of \"{$record->maildigest}\" is not one of the available options (0, 1, 2).", 0, $filename, $this->linenumber, $record, "user");
            return false;
        }
        ...
        if (!$this->validate_fixed_list($record, 'autosubscribe', array(0, 1),
                                        array('no' => 0, 'yes' => 1))) {
            $this->fslogger->log_failure("autosubscribe value of \"{$record->autosubscribe}\" is not one of the available options (0, 1).", 0, $filename, $this->linenumber, $record, "user");
            return false;
        }
        ...
        if (!$this->validate_fixed_list($record, 'trackforums', array(0, 1),
                                        array('no' => 0, 'yes' => 1))) {
            $this->fslogger->log_failure("trackforums value of \"{$record->trackforums}\" is not one of the available options (0, 1).", 0, $filename, $this->linenumber, $record, "user");
            return false;
        }
        ...
        if (!$this->validate_fixed_list($record, 'screenreader', array(0, 1),
                                        array('no' => 0, 'yes' => 1))) {
            $this->fslogger->log_failure("screenreader value of \"{$record->screenreader}\" is not one of the available options (0, 1).", 0, $filename, $this->linenumber, $record, "user");
            return false;
        }
        ...
        if (!$this->validate_fixed_list($record, 'country',
            ...
            $this->fslogger->log_failure("country value of \"{$record->country}\" is not a valid country or country code.", 0, $filename, $this->linenumber, $record, "user");
            return false;
        }
        ...
        if (isset($record->timezone)) {
            if ($CFG->forcetimezone != 99 && $record->timezone != $CFG->forcetimezone) {
                $this->fslogger->log_failure("timezone value of \"{$record->timezone}\" is not consistent with forced timezone value of \"{$CFG->forcetimezone}\" on your site.", 0, $filename, $this->linenumber, $record, "user");
                return false;
            }
        }
        ...
        if (!$this->validate_fixed_list($record, 'timezone', array_keys($timezones))) {
            $this->fslogger->log_failure("timezone value of \"{$record->timezone}\" is not a valid timezone.", 0, $filename, $this->linenumber, $record, "user");
            return false;
        }
        ...
        if (!$this->validate_fixed_list($record, 'theme', array_keys($themes))) {
            $this->fslogger->log_failure("theme value of \"{$record->theme}\" is not a valid theme.", 0, $filename, $this->linenumber, $record, "user");
            return false;
        }
        ...
        if (!$this->validate_fixed_list($record, 'lang', array_keys($languages))) {
            $this->fslogger->log_failure("lang value of \"{$record->lang}\" is not a valid language code.", 0, $filename, $this->linenumber, $record, "user");
            return false;
        }
    ...
    function user_update($record, $filename) {
        ...
        if (isset($record->username)) {
            ...
            if(!$updateusername) {
                $this->fslogger->log_failure("username value of \"{$params['username']}\" does not refer to a valid user.", 0, $filename, $this->linenumber, $record, "user");
                return false;
            }
        }
        ...
        if (isset($record->email)) {
            ...
            if(!$updateemail) {
                $this->fslogger->log_failure("email value of \"{$params['email']}\" does not refer to a valid user.", 0, $filename, $this->linenumber, $record, "user");
                return false;
            }
        }
        ...
        if (isset($record->idnumber)) {
            ...
            if(!$updateidnumber) {
                $this->fslogger->log_failure("idnumber value of \"{$params['idnumber']}\" does not refer to a valid user.", 0, $filename, $this->linenumber, $record, "user");
                return false;
            }
        }
    ...
    function validate_core_course_data($action, $record, $filename) {
        ...
        $themes = get_list_of_themes();
        if (!$this->validate_fixed_list($record, 'theme', array_keys($themes))) {
            $this->fslogger->log_failure("theme value of \"{$record->theme}\" is not a valid theme.", 0, $filename, $this->linenumber, $record, "course");
            return false;
        }
        ...
        if (isset($record->format)) {
            ...
            if (!$this->validate_fixed_list($record, 'format', array_keys($courseformats))) {
                $this->fslogger->log_failure("format value of \"{$record->format}\" does not refer to a valid course format.", 0, $filename, $this->linenumber, $record, "course");
                return false;
            }
        }
        ...
        if (isset($record->numsections)) {
            ...
            if ($record->numsections < 0 || $record->numsections > $maxsections) {
                $this->fslogger->log_failure("numsections value of \"{$record->numsections}\" is not one of the available options (0 .. {$maxsections}).", 0, $filename, $this->linenumber, $record, "course");
        ...
        if (!$this->validate_fixed_list($record, 'newsitems', $options)) {
            $this->fslogger->log_failure("newsitems value of \"{$record->newsitems}\" is not one of the available options (0 .. 10).", 0, $filename, $this->linenumber, $record, "course");
            return false;
        }
        ...
        if (!$this->validate_fixed_list($record, 'showgrades', array(0, 1),
                                        array('no' => 0, 'yes' => 1))) {
            $this->fslogger->log_failure("showgrades value of \"{$record->showgrades}\" is not one of the available options (0, 1).", 0, $filename, $this->linenumber, $record, "course");
            return false;
        }
        ...
        if (!$this->validate_fixed_list($record, 'showreports', array(0, 1),
                                        array('no' => 0, 'yes' => 1))) {
            $this->fslogger->log_failure("showreports value of \"{$record->showreports}\" is not one of the available options (0, 1).", 0, $filename, $this->linenumber, $record, "course");
            return false;
        }
        ...
        if (isset($record->maxbytes)) {
            ...
            if (!$this->validate_fixed_list($record, 'maxbytes', array_keys($choices))) {
                $this->fslogger->log_failure("maxbytes value of \"{$record->maxbytes}\" is not one of the available options.", 0, $filename, $this->linenumber, $record, "course");
                return false;
            }
        }
        ...
        if (!$this->validate_fixed_list($record, 'guest', array(0, 1),
                                        array('no' => 0, 'yes' => 1))) {
            $this->fslogger->log_failure("guest value of \"{$record->guest}\" is not one of the available options (0, 1).", 0, $filename, $this->linenumber, $record, "course");
            return false;
        }
        ...
        if (!$this->validate_fixed_list($record, 'visible', array(0, 1),
                                        array('no' => 0, 'yes' => 1))) {
            $this->fslogger->log_failure("visible value of \"{$record->visible}\" is not one of the available options (0, 1).", 0, $filename, $this->linenumber, $record, "course");
            return false;
        }
        ...
        if (!$this->validate_fixed_list($record, 'lang', $language_codes)) {
            $this->fslogger->log_failure("lang value of \"{$record->lang}\" is not a valid language code.", 0, $filename, $this->linenumber, $record, "course");
            return false;
        }
    ...
    function course_update($record, $filename) {
        ...
        if (empty($record->id)) {
            $this->fslogger->log_failure("shortname value of \"{$record->shortname}\" does not refer to a valid course.", 0, $filename, $this->linenumber, $record, "course");
            return false;
        }
