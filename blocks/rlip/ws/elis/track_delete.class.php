 ...
 */
class block_rldh_elis_track_delete extends external_api {
    ...
    public static function track_delete(array $data) {
        ...
        if (empty($data->idnumber) || !($trkid = $DB->get_field(track::TABLE, 'id', array('idnumber' => $data->idnumber)))) {
            throw new data_object_exception('ws_track_delete_fail_invalid_idnumber', 'block_rlip', '', $data);
        }

        // Capability checking.
        require_capability('elis/program:track_delete', context_elis_track::instance($trkid));
        ...
        if (!$DB->record_exists(track::TABLE, array('idnumber' => $data->idnumber))) {
            return array(
                'messagecode' => get_string('ws_track_delete_success_code', 'block_rlip'),
                'message' => get_string('ws_track_delete_success_msg', 'block_rlip')
            );
        } else {
            throw new data_object_exception('ws_track_delete_fail', 'block_rlip');
        }
    }
