 ...
 */
class block_rldh_elis_course_delete extends external_api {
    ...
    public static function course_delete(array $data) {
        ...
        if (!($crsid = $DB->get_field(course::TABLE, 'id', array('idnumber' => $data->idnumber)))) {
            throw new data_object_exception('ws_course_delete_fail_invalid_idnumber', 'block_rlip', '', $data);
        }

        // Capability checking.
        require_capability('elis/program:course_delete', context_elis_course::instance($crsid));
        ...
        if (!$DB->record_exists(course::TABLE, array('id' => $crsid))) {
            return array(
                'messagecode' => get_string('ws_course_delete_success_code', 'block_rlip'),
                'message' => get_string('ws_course_delete_success_msg', 'block_rlip')
            );
        } else {
            throw new data_object_exception('ws_course_delete_fail', 'block_rlip');
        }
    }
