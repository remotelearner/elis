 ...
 */
class local_datahub_elis_program_enrolment_delete extends external_api {
    ...
    public static function program_enrolment_delete(array $data) {
        ...
        if (!empty($id) && ($stucur = new curriculumstudent($id))) {
            $stucur->delete();
            return array(
                'messagecode' => get_string('ws_program_enrolment_delete_success_code', 'local_datahub'),
                'message' => get_string('ws_program_enrolment_delete_success_msg', 'local_datahub'),
            );
        } else {
