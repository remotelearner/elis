...
class block_rldh_elis_track_enrolment_create extends external_api {
     ...
     */
    public static function get_track_enrolment_input_object_description() {
        return array(
    ...
    /**
     * Gets a description of the track enrolment object returned by functions.
     * @return array An array of external_value objects describing a user record in webservice terms.
     */
    public static function get_track_enrolment_output_object_description() {
        return array(
            'id' => new external_value(PARAM_INT, 'The user_track db id', VALUE_OPTIONAL),
            'userid' => new external_value(PARAM_INT, 'The user db id', VALUE_OPTIONAL),
            'trackid' => new external_value(PARAM_FLOAT, 'The track db id', VALUE_OPTIONAL),
        );
    }
     ...
     */
    public static function track_enrolment_create_parameters() {
        $params = array('data' => new external_single_structure(static::get_track_enrolment_input_object_description()));
    ...
    public static function track_enrolment_create(array $data) {
        ...
        // Capability checking.
        require_capability('elis/program:track_enrol', context_elis_track::instance($trackid));
        ...
        if ($userid == false) {
            ...
            throw new data_object_exception('ws_track_enrolment_create_fail_invalid_user', 'block_rlip', '', $a);
        }
        ...
        if (usertrack::enrol($userid, $trackid)) {
            ...
            return array(
                'messagecode' => get_string('ws_track_enrolment_create_success_code', 'block_rlip'),
                'message' => get_string('ws_track_enrolment_create_success_msg', 'block_rlip'),
                'record' => (array)$usertrackrecord,
            );
        } else {
            throw new data_object_exception('ws_track_enrolment_create_fail', 'block_rlip');
        }
    }
     ...
     */
    public static function track_enrolment_create_returns() {
        return new external_single_structure(
                array(
                    'messagecode' => new external_value(PARAM_TEXT, 'Response Code'),
                    'message' => new external_value(PARAM_TEXT, 'Response'),
                    'record' => new external_single_structure(static::get_track_enrolment_output_object_description())
                )
        );
    }
}
