 ...
 */
function xmldb_dhimport_version1_uninstall() {
    global $DB;
    $dbman = $DB->get_manager();

    //remove the field mapping table
    $mapping_table = new xmldb_table(RLIPIMPORT_VERSION1_MAPPING_TABLE);
    if ($dbman->table_exists($mapping_table)) {
        $dbman->drop_table($mapping_table);
    }

    //clear config_plugins entries
    unset_all_config_for_plugin('dhimport_version1');
