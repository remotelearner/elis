 ...
 */
function xmldb_dhexport_version1elis_uninstall() {
    global $DB;
    $dbman = $DB->get_manager();

    //remove the custom field mapping table
    $mapping_table = new xmldb_table(RLIPEXPORT_VERSION1ELIS_FIELD_TABLE);
    if ($dbman->table_exists($mapping_table)) {
        $dbman->drop_table($mapping_table);
    }

    //clear config_plugins entries
    unset_all_config_for_plugin('dhexport_version1elis');
