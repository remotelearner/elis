...
class courseprerequisite_testcase extends elis_database_test {
    ...
    protected function load_csv_data() {
        $dataset = $this->createCsvDataSet(array(
            courseprerequisite::TABLE => elispm::file('tests/fixtures/course_prerequisite.csv'),
        ));
     ...
     */
    public function test_courseprerequisite_hascorrectdbfields() {
        $testobj = new courseprerequisite(false, null, array(), false, array());
     ...
     */
    public function test_courseprerequisite_hascorrectassociations() {
        $testobj = new courseprerequisite(false, null, array(), false, array());
     ...
     */
    public function test_courseprerequisite_cancreaterecord() {
        // Create a record.
        $src = new courseprerequisite(false, null, array(), false, array());
        $src->curriculumcourseid = 1;
        $src->courseid = 2;
        $src->save();

        // Read it back.
        $retr = new courseprerequisite($src->id, null, array(), false, array());
     ...
     */
    public function test_courseprerequisite_canupdaterecord() {
        $this->load_csv_data();

        // Read a record.
        $src = new courseprerequisite(3, null, array(), false, array());
        $src->courseid = 4;
        $src->save();

        // Read it back.
        $retr = new courseprerequisite(3, null, array(), false, array());
        foreach ($src as $key => $value) {
            if (strpos($key, elis_data_object::FIELD_PREFIX) !== false) {
                $key = substr($key, strlen(elis_data_object::FIELD_PREFIX));
                $this->assertEquals($src->{$key}, $retr->{$key});
