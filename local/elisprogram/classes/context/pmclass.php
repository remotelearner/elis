...
/**
 * ELIS Class Instance Context.
 */
class pmclass extends \local_eliscore\context\base {
    /**
     * Please use \local_elisprogram\context\pmclass::instance($classid) if you need the instance of this context.
    ...
    protected function __construct(\stdClass $record) {
        ...
        if ($record->contextlevel != \local_eliscore\context\helper::get_level_from_class_name(get_class($this))) {
            throw new \coding_exception('Invalid $record->contextlevel in \local_elisprogram\context\pmclass constructor.');
        }
    }
     ...
     *
     * @return \local_elisprogram\context\pmclass|array A context instance or an empty array
     */
    ...
    protected static function build_paths($force) {
        ...
        if ($force or $DB->record_exists_select('context', "contextlevel = ".$contextlevel." AND (depth = 0 OR path IS NULL)")) {
            ...
            $parentcontextlevel = \local_eliscore\context\helper::get_level_from_class_name('\local_elisprogram\context\course');

            $sql = "INSERT INTO {context_temp} (id, path, depth)
                    SELECT ctx.id, ".$DB->sql_concat('pctx.path', "'/'", 'ctx.id').", pctx.depth+1
                      FROM {context} ctx
                      JOIN {local_elisprogram_cls} cls ON (cls.id = ctx.instanceid AND ctx.contextlevel = ".$contextlevel.")
                      JOIN {context} pctx ON (pctx.instanceid = cls.courseid AND pctx.contextlevel = ".$parentcontextlevel.")
