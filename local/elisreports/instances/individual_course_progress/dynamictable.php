...
require_once($CFG->dirroot .'/local/elisreports/instances/'. $report_name .'/'.
...
if (is_array($fieldidlist)) {
    ...
    foreach ($fieldidlist as $field_id) {
        ...
        $moveupdown = '<input type="image" src="'. $CFG->wwwroot .'/local/elisreports/pix/'. $image_filename . '" '.
                ' onclick="customfields_updateTable(\''. $block_id .'\',\'down\',\''.
                $CFG->wwwroot .'/local/elisreports/instances/' . $report_name .'/\',\''.
        ...
        $remove = '<input type="image" src="'. $CFG->wwwroot .'/local/elisreports/pix/'. $image_filename .'" '.
