...
/////////////////////////////////////////////////////////////////////
    ...
    function display($id = 0) {
        ...
        if ($id !== 0) {
            $this->effective_url = $CFG->wwwroot . '/local/elisreports/dynamicreport.php?id=' . $id;
        } else {
        ...
        //no data section
        if (empty($this->data)) {
            $output .= '<p align="center">' . get_string('no_report_data', 'local_elisreports') . '</p>';
            return $output;
        }
    ...
    function add_grouping_table_row($data, $spanrow, $firstcolumn, $rowclass, $currentkey) {
        ...
        $this->table->rowclass[] = $rowclass.get_string('group_class_id', 'local_elisreports').$currentkey;
    }
                ...
                if (!empty($this->ishorizontalbar[$id])) {
                    ...
                    if (is_null($datum->$effective_id)) {
                        ...
                        if ($export_format == php_report::$EXPORT_FORMAT_PDF) {
                            ...
                            $row[$effective_id] = '<img src="' . $CFG->wwwroot . '/local/elisreports/horizontalbar.php?' . implode('&', $parameters) . '">';
                        }
                    }
                } else {
    ...
    function download($format, $query, $params, $storage_path = NULL) {
        ...
        switch ($format) {
            case php_report::$EXPORT_FORMAT_CSV:
                require_once($CFG->dirroot . '/local/elisreports/export/php_report_export_csv.class.php');
                $export = new php_report_export_csv($this);
                $export->export($query, $params, $storage_path, $filename);
                break;

            case php_report::$EXPORT_FORMAT_EXCEL:
                require_once($CFG->dirroot . '/local/elisreports/export/php_report_export_excel.class.php');
                $export = new php_report_export_excel($this);
                $export->export($query, $params, $storage_path, $filename);
                break;

            case php_report::$EXPORT_FORMAT_PDF:
                require_once($CFG->dirroot . '/local/elisreports/export/php_report_export_pdf.class.php');
...
function print_table($table, $return=false) {
    ...
    function get_lastload_display($id = 0) {
        ...
        return '<form id="'.$element_id.'" action="'.$CFG->wwwroot.'/local/elisreports/dynamicreport.php" >'.
               ...
               '<p align="center" class="php_report_caching_info">'. get_string('infocurrent', 'local_elisreports', $a) .'<br/>'.
