...
abstract class gas_gauge_table_report extends table_report {
    ...
    function print_gas_gauge($id) {
        if (!empty($this->total_field)) {
            ...
            return '<img src="'. $CFG->wwwroot .'/local/elisreports/gas_gauge_output.php?value='. $this->gas_gauge_value .'&total='. $this->gas_gauge_max_value . '&radius='. $radius .'&palette='. urlencode(base64_encode(serialize($palette))) .'" class="php_report_gas_gauge_image"/>';
        }
        return '<br/>';
    }
