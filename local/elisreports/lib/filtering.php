    ...
    if (!empty($_GET['report']) && $_GET['report'] == $report_shortname) {
        ...
        foreach ($urlparams as $key => $val) {
           /*
            ...
            $tmp = ob_get_contents();
            ob_end_clean();
            error_log("/local/elisreports/lib/filtering.php::php_report_filtering_get_user_preferences($report_shortname) over-riding filter values with URL param: {$key} = {$tmp}");
           */
...
class php_report_default_capable_filtering extends generalized_filtering {
    ...
    protected function get_preferences() {
        ...
        foreach ($user_preferences as $key => $value) {
            //error_log("/local/elisreports/lib/filtering.php::get_preferences(); {$key} => {$value}");
