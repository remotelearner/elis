...
function elis_files_delete($uuid, $recursive = false, $repo = NULL) {
    ...
    // ELIS-1102
    $result = elis_files_request('/moodle/nodeowner/' . $uuid . '?username=' . $CFG->repository_elis_files_server_username);
...
/*
    ...
    if (elis_files_get_repository_version('3.3')) {
        $data .= '      <cmis:propertyString cmis:name="folderName">' . "\n" .
                 '        <cmis:value>' . $name . '</cmis:value>' . "\n" .
                 '      </cmis:propertyString>' . "\n";
    }

    $data .= '    </cmis:properties>' . "\n" .
             '  </cmis:object>' . "\n" .
             '</entry>';
*/
...
//    if (elis_files_get_repository_version('3.3')) {
//        $uri = '/api/node/workspace/SpacesStore/' . $uuid . '/children';
//    } else {
