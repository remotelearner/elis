...
function xmldb_repository_elis_files_upgrade($oldversion = 0) {
    ...
    if ($oldversion < 2012042300) {
        ...
        if ($dbman->table_exists('elis_files_organization_store') && !$dbman->table_exists('elis_files_userset_store')) {
            ...
            $dbman->drop_index($original_table, $original_uuid_index);
            $dbman->drop_index($original_table, $original_index);

            $new_table = new xmldb_table('elis_userset_store');

            //rename fields
            $organization = new xmldb_field('organizationid', XMLDB_TYPE_INTEGER, '10', XMLDB_UNSIGNED, XMLDB_NOTNULL, XMLDB_SEQUENCE, '0');
