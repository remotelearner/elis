...
if (!$repo) {
    mtrace(get_string('couldnotinitializerepository', 'repository_elis_files'));
    exit;
}
...
if (!empty($rs)) {
    ...
    mtrace("\n".get_string('np_startingmigration', 'repository_elis_files', $totalusers)."\n");
    ...
    foreach ($rs as $user) {
        ...
        if (!$migrate_ok) {
            $errors++;
            mtrace(get_string('np_migratefailure', 'repository_elis_files', $a));
        } else {
            mtrace(get_string('np_migratesuccess', 'repository_elis_files', $a));
        }
    }
}
...
mtrace("\n".get_string('np_migratecomplete', 'repository_elis_files', $a)."\n");
